...
class Ranking (pl.LightningModule):
    ...
    def forward(self, img):
        ...
        self.backbone(img1)
        vis1, _ = self.get_activations()
        self.backbone(img2)
        vis2, which_layer = self.get_activations() #which layer can come from vis1 or vis2, they're equal
        ranking_input = torch.cat((vis1, vis2))
        which_layer+=which_layer #duplicate the entries so which_layer[i] corresponds to ranking_input[i]
        print (ranking_input.shape)
        print (which_layer)
        print (len(which_layer))
        #code must be added from here on
        exit()
